...
kernle -s my-project sync push

# Push up to 10 changes
kernle -s my-project sync push --limit 10
```
...
```bash
# Pull recent changes
kernle -s my-project sync pull

# Full sync from cloud
kernle -s my-project sync pull --full
```
...
```bash
kernle -s my-project auth register --email user@example.com
```
...
```bash
# Interactive login
kernle -s my-project auth login

# With API key
kernle -s my-project auth login --api-key knl_sk_xxxxx
```
...
# Skip sync on load
kernle -s my-project load --no-sync

# Force sync after checkpoint
kernle -s my-project checkpoint save "task" --sync

# Skip sync after checkpoint
kernle -s my-project checkpoint save "task" --no-sync
```
...
kernle -s my-project sync status
