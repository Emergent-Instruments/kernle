...
<Warning>
**Renamed Command**: `kernle consolidate` has been renamed to `kernle promote`. The old `consolidate` command still works as a deprecated alias but will emit a warning.
</Warning>
...
# Consolidation
prompt = k.promote(limit=20)  # or k.consolidate(limit=20) for backwards compat
