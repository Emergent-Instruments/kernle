...
To ensure consistent memory health across sessions, add a Kernle section to your project's `CLAUDE.md` (for Claude Code) or `AGENTS.md` (for OpenClaw). This instructs the agent to load memory and check health at the start of every session.
...
1. **Load memory:**
   kernle -s {YOUR_STACK_ID} load

2. **Check health:**
   kernle -s {YOUR_STACK_ID} anxiety

3. **If anxiety > 50:** Address before proceeding
   - Run `kernle promote` for episode debt
...
If WARN/CRITICAL, run `kernle promote` before proceeding.
```
