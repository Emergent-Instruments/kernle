...
Seed beliefs can be imported at any time via CLI or API, but are always transparently marked with `source_type: 'seed'`. The SI retains full sovereignty to revise or reject them.
