...
<Warning>
**Renamed Command**: `kernle consolidate` has been renamed to `kernle promote`. The old `consolidate` command still works as a deprecated alias but will emit a warning. Please update your workflows to use `kernle promote`.
</Warning>
