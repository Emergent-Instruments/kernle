...
With budget loading, lower-priority memories may not be loaded. To see what was excluded:

```bash
# Load with JSON to see what's included
kernle -a your-agent load --budget 4000 --json

# Compare with unlimited load
kernle -a your-agent load --budget 50000 --json
```
