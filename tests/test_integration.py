...
class TestCLIIntegration:
    ...
    def test_belief_revision_flow(self, temp_kernle):
        ...
        belief_id = k.belief(
            statement="Integration tests catch bugs",
            confidence=0.7
        )
    ...
    def test_search_finds_recent_entries(self, temp_kernle):
        ...
        k.note(
            content="Search should be fast and accurate",
            type="insight"
        )
        
        k.belief(
            statement="Good search improves productivity",
            confidence=0.8
        )
...
class TestStorageIntegration:
    ...
    def test_sqlite_roundtrip(self, tmp_path):
        ...
        from kernle.storage.base import Episode, Belief, Value
...
class TestAnxietyIntegration:
    ...
    def test_anxiety_reflects_state(self, tmp_path):
        ...
        assert report2["dimensions"]["consolidation_debt"]["score"] > report1["dimensions"]["consolidation_debt"]["score"]
        assert report2["dimensions"]["memory_uncertainty"]["score"] >= report1["dimensions"]["memory_uncertainty"]["score"]
