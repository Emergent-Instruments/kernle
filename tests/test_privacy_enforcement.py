...
class TestPrivacyFields:
    ...
    def setup_method(self):
        ...
        self.kernle = Kernle(agent_id="test-privacy", storage=self.storage, checkpoint_dir=self.tmpdir)
    ...
    def test_drive_privacy_fields_storage(self):
        ...
        drive_id = self.storage.save_drive(drive)
    ...
    def test_relationship_privacy_fields_storage(self):
        ...
        rel_id = self.storage.save_relationship(relationship)
...
class TestAccessControl:
    ...
    def test_search_respects_privacy_filters(self):
        ...
        accessible_count = len([rid for rid in result_ids if rid in ["episode_public", "episode_team"]])
        private_count = len([rid for rid in result_ids if rid in ["episode_private", "episode_empty"]])
...
class TestSchemaMigration:
    ...
    def test_privacy_columns_added_to_all_tables(self):
        ...
        with storage._connect() as conn:
            tables_to_check = [
                "episodes", "beliefs", "agent_values", "goals", 
                "notes", "drives", "relationships", "playbooks", "raw_entries"
            ]
...
class TestAllMemoryTypes:
    ...
    def test_all_memory_types_support_privacy_fields(self):
        ...
        episode = Episode(
            ...
            subject_ids=["s1"], access_grants=["a1"], consent_grants=["c1"]
        )
        episode_id = self.storage.save_episode(episode)
        ...
        belief = Belief(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", statement="Test belief",
            subject_ids=["s2"], access_grants=["a2"], consent_grants=["c2"]
        )
        belief_id = self.storage.save_belief(belief)
        ...
        value = Value(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", name="Test", statement="Test value",
        ...
        goal = Goal(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", title="Test Goal",
        ...
        note = Note(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", content="Test note",
        ...
        drive = Drive(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", drive_type="test_drive",
            subject_ids=["s6"], access_grants=["a6"], consent_grants=["c6"]
        )
        drive_id = self.storage.save_drive(drive)
        ...
        relationship = Relationship(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", entity_name="Test Entity", 
            entity_type="human", relationship_type="colleague",
            subject_ids=["s7"], access_grants=["a7"], consent_grants=["c7"]
        )
        rel_id = self.storage.save_relationship(relationship)
        ...
        playbook = Playbook(
            id=str(uuid.uuid4()),
            agent_id="test-all-types", name="Test Playbook",
            description="Test", trigger_conditions=["t1"], 
            steps=[{"action": "a1"}], failure_modes=["f1"],
            subject_ids=["s8"], access_grants=["a8"], consent_grants=["c8"]
        )
        playbook_id = self.storage.save_playbook(playbook)
    ...
    def test_backward_compatibility_with_null_privacy_fields(self):
        ...
        episode = Episode(
            id=str(uuid.uuid4()),
            agent_id="test-all-types",
            objective="Old episode", outcome="Old outcome"
