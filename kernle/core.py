...
class Kernle(
    ...
    def export_cache(
        ...
        Note:
            The path parameter should be a trusted value. This method does not
            validate that the path is within any particular directory.
        """
        # Input validation
        min_confidence = max(0.0, min(1.0, min_confidence))  # Clamp to [0, 1]
        max_beliefs = max(0, max_beliefs)  # Ensure non-negative
        ...
        if relationships:
            ...
            sorted_rels = sorted(
                relationships,
                key=lambda r: r.interaction_count if hasattr(r, 'interaction_count') else 0,
                reverse=True,
            )[:10]
            if sorted_rels:
                lines.append("## Key Relationships")
                for r in sorted_rels:
                    notes_str = f" — {r.notes[:80]}" if r.notes else ""
