...
class Kernle(
    ...
    def _export_boot_file(self) -> None:
        ...
        lines.append(f"<!-- Do not edit manually. Use: kernle -a {self.agent_id} boot set <key> <value> -->")
        lines.append("")
    ...
    def export_cache(
        ...
        if values:
            lines.append("## Values")
            for v in sorted(values, key=lambda x: x.priority if x.priority is not None else 0, reverse=True):
        ...
        if goals:
            lines.append("## Goals")
            for g in sorted(goals, key=lambda x: x.priority if x.priority is not None else 0, reverse=True):
        ...
        if relationships:
            ...
            if sorted_rels:
                ...
                for r in sorted_rels:
                    notes_str = f" — {r.notes[:80]}..." if r.notes and len(r.notes) > 80 else (f" — {r.notes}" if r.notes else "")
